...
class TestDancer:
    def test_get_all_dancers(self, test_app, dancer_id):
    ...
    def test_get_dancer(self, test_app, dancer_id):
        response = test_app.get(f"/dancers/{dancer_id}")
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == dancer_id

    def test_update_dancer(self, test_app, dancer_id):
        updated_payload = {
    ...
    def test_delete_dancer(self, test_app, dancer_id):
        # delete the dancer
