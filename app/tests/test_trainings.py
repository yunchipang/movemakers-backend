...
class TestRegistration:
    ...
    def test_register_training_successful(self):
        ...
        response1 = self.client.post(
            f"/trainings/{self.training_id}/register", headers=self.get_headers(self.tokens["user1"])
        )
        response2 = self.client.post(
            f"/trainings/{self.training_id}/register", headers=self.get_headers(self.tokens["user2"])
        )
    ...
    def test_register_training_full(self):
        ...
        response = self.client.post(f"/trainings/{self.training_id}/register", headers=self.get_headers(self.tokens["user3"]))
    ...
    def test_register_training_cancel(self):
        ...
        response = self.client.delete(
            f"/trainings/{self.training_id}/cancel", headers=self.get_headers(self.tokens["user2"])
        )
        assert response.status_code == 200
    ...
    def test_register_training_non_existent(self):
        ...
        response = self.client.post(
            f"/trainings/{non_existent_training_id}/register", headers=self.get_headers(self.tokens["user1"])
        )
        assert response.status_code == 400
        assert response.json() == {
            "detail": "Training does not exist"
        }
